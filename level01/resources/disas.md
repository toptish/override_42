...
Dump of assembler code for function main:
   ...
   // 
   0x080484d8 <+8>:	sub    $0x60,%esp                           // 96 byte на локальные переменные
   0x080484db <+11>:	lea    0x1c(%esp),%ebx                  // 0xf7fceff4 в EBX - local buffer = 64 (0x5c - 0x1c= 0x40)
   0x080484df <+15>:	mov    $0x0,%eax                        // заполняем 0
   0x080484e4 <+20>:	mov    $0x10,%edx                       // 16 шагов по 4 байта за шаг
   0x080484e9 <+25>:	mov    %ebx,%edi
   0x080484eb <+27>:	mov    %edx,%ecx
   0x080484ed <+29>:	rep stos %eax,%es:(%edi)                // memset(buffer, 0, 64)
   0x080484ef <+31>:	movl   $0x0,0x5c(%esp)                  // esp+92 int check
   ...
   0x08048521 <+81>:	movl   $0x804a040,(%esp)                // 0x804a040 <a_user_name>:	 ""
   ...
   0x0804856d <+157>:	lea    0x1c(%esp),%eax                  // buffer
   0x08048571 <+161>:	mov    %eax,(%esp)
   0x08048574 <+164>:	call   0x8048370 <fgets@plt>            // fgets(buffer, 100, stdin)
   0x08048579 <+169>:	lea    0x1c(%esp),%eax                  // buffer -> eax
   0x0804857d <+173>:	mov    %eax,(%esp)                      // buffer - argument for verify_user_pass
   0x08048580 <+176>:	call   0x80484a3 <verify_user_pass>     // verify_user_pass(buffer)
   0x08048585 <+181>:	mov    %eax,0x5c(%esp)                  // verify_user_pass return -> int check
   0x08048589 <+185>:	cmpl   $0x0,0x5c(%esp)                  // if check!=0
   0x0804858e <+190>:	je     0x8048597 <main+199>
   0x08048590 <+192>:	cmpl   $0x0,0x5c(%esp)                  // if check !=0
...
Dump of assembler code for function verify_user_name:
   ...
   0x08048478 <+20>:	mov    $0x804a040,%edx                  // 0x804a040 <a_user_name>:	 ""
   0x0804847d <+25>:	mov    $0x80486a8,%eax                  // "dat_wil"
   0x08048482 <+30>:	mov    $0x7,%ecx                        // 7
   0x08048487 <+35>:	mov    %edx,%esi
   0x08048489 <+37>:	mov    %eax,%edi
   0x0804848b <+39>:	repz cmpsb %es:(%edi),%ds:(%esi)        // strncmp('dat_wil', a_user_name, 7)
   0x0804848d <+41>:	seta   %dl
   0x08048490 <+44>:	setb   %al
   0x08048493 <+47>:	mov    %edx,%ecx
   0x08048495 <+49>:	sub    %al,%cl
   0x08048497 <+51>:	mov    %ecx,%eax
   0x08048499 <+53>:	movsbl %al,%eax                         // result of comparison to EAX
   0x0804849c <+56>:	add    $0x10,%esp                       // set back stack pointer
...
0x080484a8 <+5>:	mov    0x8(%ebp),%eax                   // funct_arg to eax
...
0x080484bb <+24>:	repz cmpsb %es:(%edi),%ds:(%esi)        // strncmp('admin', func_arg, 5)
0x080484bd <+26>:	seta   %dl                              // dl = *rdi > *rsi
0x080484c0 <+29>:	setb   %al                              // al = *rdi < *rsi
0x080484c3 <+32>:	mov    %edx,%ecx
0x080484c5 <+34>:	sub    %al,%cl
0x080484c7 <+36>:	mov    %ecx,%eax
0x080484c9 <+38>:	movsbl %al,%eax                         // save comparison result to eax
