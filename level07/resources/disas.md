...
Dump of assembler code for function main:
   0x08048723 <+0>:	push   %ebp                                 // push ebp to stack
   0x08048724 <+1>:	mov    %esp,%ebp                            // store esp in ebp
   0x08048726 <+3>:	push   %edi                                 // push edi to stack
   0x08048727 <+4>:	push   %esi                                 // push esi to stack
   0x08048728 <+5>:	push   %ebx                                 // push ebx to stack
   0x08048729 <+6>:	and    $0xfffffff0,%esp                     // alignment
   0x0804872c <+9>:	sub    $0x1d0,%esp                          // 464 bytes for local variables
   0x08048732 <+15>:	mov    0xc(%ebp),%eax                   // argv[0] to eax
   0x08048735 <+18>:	mov    %eax,0x1c(%esp)                  // argv[0] to esp+0x1c (esp+28)
   0x08048739 <+22>:	mov    0x10(%ebp),%eax                  // env var 1 ?
   0x0804873c <+25>:	mov    %eax,0x18(%esp)                  // env var to esp+0x18 (esp+24)
   0x08048740 <+29>:	mov    %gs:0x14,%eax                    // canary
   0x08048746 <+35>:	mov    %eax,0x1cc(%esp)                 // 4bytes canary to 0x1cc
   0x0804874d <+42>:	xor    %eax,%eax                        // eax = 0
   0x0804874f <+44>:	movl   $0x0,0x1b4(%esp)                 // 4 bytes var1 to esp+436 int ret=0
   0x0804875a <+55>:	movl   $0x0,0x1b8(%esp)                 // char buff[20]={}
   0x08048765 <+66>:	movl   $0x0,0x1bc(%esp)                 // ~ 
   0x08048770 <+77>:	movl   $0x0,0x1c0(%esp)                 // ~ 
   0x0804877b <+88>:	movl   $0x0,0x1c4(%esp)                 // ~ 
   0x08048786 <+99>:	movl   $0x0,0x1c8(%esp)                 // ~ (esp + 456)
   0x08048791 <+110>:	lea    0x24(%esp),%ebx                  // esp+36 - buffer[400]
   0x08048795 <+114>:	mov    $0x0,%eax
   0x0804879a <+119>:	mov    $0x64,%edx                       // 100 to edx 100 steps 4 bytes each
   0x0804879f <+124>:	mov    %ebx,%edi
   0x080487a1 <+126>:	mov    %edx,%ecx
   0x080487a3 <+128>:	rep stos %eax,%es:(%edi)                // char buf[400] = {0} memset analogue
   0x080487a5 <+130>:	jmp    0x80487ea <main+199>             // loop start
   0x080487a7 <+132>:	mov    0x1c(%esp),%eax                  // argv
   0x080487ab <+136>:	mov    (%eax),%eax                      // argv[i]
   0x080487ad <+138>:	movl   $0xffffffff,0x14(%esp)           // esp+20 counter
   ...
   0x080487c4 <+161>:	mov    %ecx,%eax                        // counter
   0x080487c6 <+163>:	not    %eax                             // 
   0x080487c8 <+165>:	lea    -0x1(%eax),%edx                  // len = counter-1
   0x080487cb <+168>:	mov    0x1c(%esp),%eax                  // argv       
   0x080487cf <+172>:	mov    (%eax),%eax                      // argv[0+i]
   0x080487d1 <+174>:	mov    %edx,0x8(%esp)                   // arg3 - len
   0x080487d5 <+178>:	movl   $0x0,0x4(%esp)                   // arg2 - 0
   0x080487dd <+186>:	mov    %eax,(%esp)                      // arg1 - argv[0+i]
   0x080487e0 <+189>:	call   0x80484f0 <memset@plt>           // memset(argv[0+i], 0, len)
   0x080487e5 <+194>:	addl   $0x4,0x1c(%esp)                  // argv++
   0x080487ea <+199>:	mov    0x1c(%esp),%eax
   0x080487ee <+203>:	mov    (%eax),%eax
   0x080487f0 <+205>:	test   %eax,%eax                        // if eax==0
   0x080487f2 <+207>:	jne    0x80487a7 <main+132>             // continue loop
   0x080487f4 <+209>:	jmp    0x8048839 <main+278>             //
   0x080487f6 <+211>:	mov    0x18(%esp),%eax                  // envp
   0x080487fa <+215>:	mov    (%eax),%eax                      // envp[0]
   0x080487fc <+217>:	movl   $0xffffffff,0x14(%esp)           // esp+20
   0x08048804 <+225>:	mov    %eax,%edx                        //
   0x08048806 <+227>:	mov    $0x0,%eax
   0x0804880b <+232>:	mov    0x14(%esp),%ecx
   0x0804880f <+236>:	mov    %edx,%edi
   0x08048811 <+238>:	repnz scas %es:(%edi),%al               // strlen(envp[0])
   0x08048813 <+240>:	mov    %ecx,%eax
   0x08048815 <+242>:	not    %eax                             //counter
   0x08048817 <+244>:	lea    -0x1(%eax),%edx                  // len_envp = counter-1
   0x0804881a <+247>:	mov    0x18(%esp),%eax
   0x0804881e <+251>:	mov    (%eax),%eax                      // envp[0+i]
   0x08048820 <+253>:	mov    %edx,0x8(%esp)                   // arg3 - len_envp
   0x08048824 <+257>:	movl   $0x0,0x4(%esp)                   // arg2 - 0
   0x0804882c <+265>:	mov    %eax,(%esp)                      // arg1 - envp[0]
   0x0804882f <+268>:	call   0x80484f0 <memset@plt>
   0x08048834 <+273>:	addl   $0x4,0x18(%esp)                  // envp++
   0x08048839 <+278>:	mov    0x18(%esp),%eax
   0x0804883d <+282>:	mov    (%eax),%eax
   0x0804883f <+284>:	test   %eax,%eax                        // if envp==0
   0x08048841 <+286>:	jne    0x80487f6 <main+211>             // continue loop
   ...
   0x0804884f <+300>:	mov    $0x8048d4b,%eax                  // ARG1 -   "Input command: "
   ...
   0x0804886c <+329>:	mov    %eax,0x8(%esp)                   // arg3 - stdin
   0x08048870 <+333>:	movl   $0x14,0x4(%esp)                  // arg2 - 20
   0x08048878 <+341>:	lea    0x1b8(%esp),%eax                 // arg1 - buf at esp+1b8
   ...
   0x080488a3 <+384>:	repnz scas %es:(%edi),%al               // strlen buf
   0x080488a5 <+386>:	mov    %ecx,%eax
   0x080488a7 <+388>:	not    %eax
   0x080488a9 <+390>:	sub    $0x1,%eax                        // strlen(buf)
   0x080488ac <+393>:	sub    $0x1,%eax                        // strlen(buf) - 1
   0x080488af <+396>:	movb   $0x0,0x1b8(%esp,%eax,1)          // buf[strlen(buf) - 1] = 0;
   ...
   0x080488ce <+427>:	repz cmpsb %es:(%edi),%ds:(%esi)        // strncmp("store", buf, 5)
   ...
   0x080488e1 <+446>:	jne    0x80488f8 <main+469>             // jump not equal
   0x080488e3 <+448>:	lea    0x24(%esp),%eax                  // buffer 
   0x080488e7 <+452>:	mov    %eax,(%esp)
   0x080488ea <+455>:	call   0x8048630 <store_number>         // store_number(buffer)
   0x080488ef <+460>:	mov    %eax,0x1b4(%esp)                 // ret = store_number(buffer)
   0x080488f6 <+467>:	jmp    0x8048965 <main+578>
   0x080488f8 <+469>:	lea    0x1b8(%esp),%eax                 // buf
   ...
   0x0804890f <+492>:	repz cmpsb %es:(%edi),%ds:(%esi)        // strncmp("read", buf, 4)
   ...
   0x08048922 <+511>:	jne    0x8048939 <main+534>             // jump not equal
   0x08048924 <+513>:	lea    0x24(%esp),%eax                  // buffer
   0x08048928 <+517>:	mov    %eax,(%esp)
   0x0804892b <+520>:	call   0x80486d7 <read_number>          // read_number(buffer)
   0x08048930 <+525>:	mov    %eax,0x1b4(%esp)                 // ret = read_number(buffer)
   0x08048937 <+532>:	jmp    0x8048965 <main+578>
   0x08048939 <+534>:	lea    0x1b8(%esp),%eax                 // buf
   ...
   0x08048950 <+557>:	repz cmpsb %es:(%edi),%ds:(%esi)        // strncmp("quit", buf, 4)
   ...
   0x08048963 <+576>:	je     0x80489cf <main+684>             // jump equal
   0x08048965 <+578>:	cmpl   $0x0,0x1b4(%esp)                 // if ret == 0
   0x0804896d <+586>:	je     0x8048989 <main+614>             // jump equal
   0x0804896f <+588>:	mov    $0x8048d6b,%eax                  // " Failed to do %s command\n"
   0x08048974 <+593>:	lea    0x1b8(%esp),%edx                 // buf
   0x0804897b <+600>:	mov    %edx,0x4(%esp)
   0x0804897f <+604>:	mov    %eax,(%esp)
   0x08048982 <+607>:	call   0x8048470 <printf@plt>           // printf(buf)
   0x08048987 <+612>:	jmp    0x80489a1 <main+638>
   0x08048989 <+614>:	mov    $0x8048d88,%eax                  // " Completed %s command successfully\n"
   0x0804898e <+619>:	lea    0x1b8(%esp),%edx                 // buf
   0x08048995 <+626>:	mov    %edx,0x4(%esp)
   0x08048999 <+630>:	mov    %eax,(%esp)
   0x0804899c <+633>:	call   0x8048470 <printf@plt>           // printf(buf)
   0x080489a1 <+638>:	lea    0x1b8(%esp),%eax                 // buf -> eax
   0x080489a8 <+645>:	movl   $0x0,(%eax)                      // buf = {}
   ...
   0x080489dc <+697>:	xor    %gs:0x14,%esi                    // canary check
...
Dump of assembler code for function read_number:
   ...
   0x080486da <+3>:	sub    $0x28,%esp                       // 40 bytes for local vars
   0x080486dd <+6>:	movl   $0x0,-0xc(%ebp)                  // int num? ebp-12
   ...
   0x080486f6 <+31>:	mov    %eax,-0xc(%ebp)              // num = get_unum()
   0x080486f9 <+34>:	mov    -0xc(%ebp),%eax
   0x080486fc <+37>:	shl    $0x2,%eax                    // num shift left 2
   0x080486ff <+40>:	add    0x8(%ebp),%eax               // func_argument address to eax
   0x08048702 <+43>:	mov    (%eax),%edx                  // dereference
   0x08048704 <+45>:	mov    $0x8048b1b,%eax              // " Number at data[%u] is %u\n"
   0x08048709 <+50>:	mov    %edx,0x8(%esp)               // arg3 - func_arg index buffer
   0x0804870d <+54>:	mov    -0xc(%ebp),%edx              // num
   0x08048710 <+57>:	mov    %edx,0x4(%esp)               // arg2 - num
   0x08048714 <+61>:	mov    %eax,(%esp)                  // arg1 " Number at data[%u] is %u\n"
...
Dump of assembler code for function store_number:
   ...
   0x08048633 <+3>:	sub    $0x28,%esp                               // 40 bytes for local vars
   0x08048636 <+6>:	movl   $0x0,-0x10(%ebp)                         // unsigned int num = 0
   ...
   0x0804868d <+93>:	shr    $0x18,%eax                           // shift right >> 0x18
   0x08048690 <+96>:	cmp    $0xb7,%eax                           // nb >> 0x18 == 0xb7
